...
const CartScreen = ({ navigation }) => {
    ...
    // Animation values
    const cartItemAnimations = useRef([]).current;
    ...
    // Animate cart items when loaded
    useEffect(() => {
        if (cartData?.items?.length > 0) {
            cartItemAnimations.length = 0;
            cartData.items.forEach((_, index) => {
                cartItemAnimations[index] = new Animated.Value(0);
            });

            const animations = cartData.items.map((_, index) => {
                return Animated.timing(cartItemAnimations[index], {
                    toValue: 1,
                    duration: 400,
                    delay: index * 80,
    ...
    // Animate order cards when loaded
    useEffect(() => {
        const orders = activeTab === 'Current Order' ? currentOrders : previousOrders;
        if (orders?.length > 0) {
            orderAnimations.length = 0;
            orders.forEach((_, index) => {
                orderAnimations[index] = new Animated.Value(0);
            });

            const animations = orders.map((_, index) => {
                return Animated.timing(orderAnimations[index], {
                    toValue: 1,
                    duration: 400,
                    delay: index * 80,
                    useNativeDriver: true,
    ...
    }, [currentOrders, previousOrders, activeTab]);

    // Continuous blink animation for download invoice button
    useEffect(() => {
    ...
    // Checkout button press animation
...
`
        ...
        return (
            <View style={styles.cartContainer}>
                ...
                <View style={styles.productSection}>
                    ...
                    {cartData.items.map((item, index) => {
                        const animatedValue = cartItemAnimations[index] || new Animated.Value(1);
    ...
    const renderOrderCard = (order, isCurrentOrder = true, index = 0) => {
        const statusColor = getStatusColor(order.orderStatus);
        const orderDate = new Date(order.createdAt).toLocaleDateString();
        const animatedValue = orderAnimations[index] || new Animated.Value(1);
        const opacity = animatedValue;
        const translateY = animatedValue.interpolate({
            inputRange: [0, 1],
            outputRange: [30, 0],
    ...
    const renderCurrentOrderContent = () => {
        ...
        return (
            <View>
                {currentOrders.map((order, index) => renderOrderCard(order, true, index))}
            </View>
        );
    };
    ...
    const renderPreviousOrderContent = () => {
        ...
        return (
            <View>
                {previousOrders.map((order, index) => renderOrderCard(order, false, index))}
            </View>
        );
    };
                            ...
                            <Animated.View style={{ transform: [{ scale: checkoutButtonScale }] }}>
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        ...
                                        navigation.navigate('CheckoutSummary', { cartData: cartData });
                                    }}
                                >
                                    <Text style={styles.checkoutText}>Checkout</Text>
                                </TouchableOpacity>
