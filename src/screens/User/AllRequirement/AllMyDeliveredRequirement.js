...
export default function AllMyDeliveredRequirements({ navigation }) {
    const { isDarkMode } = useSelector(state => state.theme);
    const [allMyDeliveredRequirement, setAllMyDeliveredRequirement] = useState([]);
    const [isUploading, setIsUploading] = useState(false);
    const [refreshing, setRefreshing] = useState(false);

    const headerAnim = useRef(new Animated.Value(-100)).current;

    const getAllMyDeliveredRequirements = async () => {
        try {
            setIsUploading(true);
            const response = await apiGet('/api/user/GetMydeliveredRequirements');
            setAllMyDeliveredRequirement(response.data || []);
    ...
    const onRefresh = async () => {
        setRefreshing(true);
        await getAllMyDeliveredRequirements();
        setRefreshing(false);
    };
    ...
    const renderHeader = () => (
        <Animated.View style={[styles.headerContainer, { transform: [{ translateY: headerAnim }] }]}>
            ...
            <View style={styles.topBar}>
                ...
                <Text style={styles.headerTitle}>📦 All Delivered Requirements</Text>
            </View>
        </Animated.View>
    );
                    ...
                    <View style={styles.bottomRow}>
                        <View style={styles.detailsSection}>
                            ...
                            <View style={styles.priceSection}>
                                ...
                                <Text style={styles.priceText}>
                                    💸 <Text style={{ fontFamily: FONTS_FAMILY.Poppins_SemiBold }}>Delivered:</Text> ₹{req?.deliveredAmount || 0}
                                </Text>
                                <Text style={styles.priceText}>
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                <View style={styles.contentContainer}>
                    {allMyDeliveredRequirement?.length === 0 ? (
                        <View style={styles.emptyContainer}>
                            <Text style={{ fontSize: 48 }}>🚫</Text>
                            <Text style={styles.emptyText}>No delivered project found</Text>
                        </View>
                    ) : (
                        allMyDeliveredRequirement?.map((req, index) => renderRequirementItem(req, index))
                    )}
                </View>
