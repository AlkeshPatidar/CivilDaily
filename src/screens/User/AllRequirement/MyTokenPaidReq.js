...
import { apiGet, Get_Api } from '../../../utils/Apis';
import { BackIcon } from '../../../assets/SVGs';
import Ionicons from 'react-native-vector-icons/Ionicons';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import { useSelector } from 'react-redux';
// import PayRemainingPaymentModal from './PayRemainingPaymentModal';

export default function AllMyTokenPaidRequirements({ navigation }) {
    const { isDarkMode } = useSelector(state => state.theme);
    const [allMyTokenPaidRequirements, setAllMyTokenPaidRequirements] = useState([]);
    const [isUploading, setIsUploading] = useState(true);
    const [refreshing, setRefreshing] = useState(false);
    const [paymentModal, setPaymentModal] = useState(false);
    const [selectedRequirementId, setSelectedRequirementId] = useState(null);
    const [selectedRequirementTotal, setSelectedRequirementTotal] = useState(null);
    const [selectedRequirementRemainingAmount, setSelectedRequirementRemainingAmount] = useState(null);
    const [paymentDetails, setPaymentDetails] = useState(null);

    const headerAnim = useRef(new Animated.Value(-100)).current;

    const getAllMyTokenPaidRequirements = async () => {
        try {
            setIsUploading(true);
            const response = await apiGet('/api/user/GetMytokenpaidRequirements');
            setAllMyTokenPaidRequirements(response.data);
    ...
    const getPaymentDetails = async () => {
        try {
            const response = await Get_Api('/api/admin/GetPaymentDetail');
    ...
    const openModel = (requirement) => {
        ...
        setSelectedRequirementRemainingAmount(requirement?.remainingAmount);
    };
    ...
    const closeModel = () => {
        ...
        setSelectedRequirementRemainingAmount(null);
        getAllMyTokenPaidRequirements();
    };
    ...
    const renderHeader = () => (
        <Animated.View style={[styles.headerContainer, { transform: [{ translateY: headerAnim }] }]}>
            ...
            <View style={styles.topBar}>
                ...
                <Text style={styles.headerTitle}>📦 All Token Paid Requirements</Text>
            </View>
        </Animated.View>
    );
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                <View style={styles.contentContainer}>
                    {allMyTokenPaidRequirements?.length === 0 ? (
                        <View style={styles.emptyContainer}>
                            <Text style={{ fontSize: 48 }}>🚫</Text>
                            <Text style={styles.emptyText}>No Token Paid project found</Text>
                        </View>
                    ) : (
                        allMyTokenPaidRequirements?.map((req, index) => renderRequirementItem(req, index))
                    )}
                </View>
                <View style={{ height: 20 }} />
            </ScrollView>

            {/* <PayRemainingPaymentModal
                visible={paymentModal}
                onClose={closeModel}
                totalAmount={selectedRequirementTotal}
                remainingAmount={selectedRequirementRemainingAmount}
